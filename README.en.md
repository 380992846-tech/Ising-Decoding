...
export NV_ISING_CODECODE_DIR="$(pwd)/ising-decoding/code"   # or pass --nvidia-code-dir <repo>/code
```
The triangular color-code memory circuit is generated by NVIDIA's tested
`build_color_memory_circuit` (from that repo), so `--code color_code` needs the
NVIDIA repo cloned and its `code/` dir importable. See
[`docs/installation.md`](docs/installation.md) for the full setup checklist.
