...
- [ ] 实现性能对比脚本 `scripts/quantum_bench.py`（采用通用的确定性/可复现/落盘 benchmark 结构），
      把"对比 pyMatching"当成另一种"量化对比"；
- [ ] 量化提升：
  - **速度**：目标 ≥ 2.5×；
  - **精度**：目标 ≥ 3×（逻辑错误率降低）。

**方法**：基准测试采用通用的"确定性、可复现、结果落盘"做法（脚本见 `scripts/quantum_bench.py`）。

### 第二阶段：硬核系统集成

**目标**：将预解码器嵌入一个模拟的**量子控制环路**，研究**延迟预算**下的权衡。

- [ ] 新建 `quantum_control/` 目录，写一个模拟解码服务（可用 C++/Go 或 Python 异步 I/O 实现）；
- [ ] 对比**速度优化版（0.9M 参数）**和**精度优化版（1.8M 参数）**
      在不同延迟要求（如 <1ms, <10ms）下的表现；
- [ ] 批量推理与并发解码请求的调度，参考通用的 continuous-batch server 思路（如 vLLM 的 continuous batching）处理。
